...
const createMainNewsItem = (item) => {
    ...
    source.classList.add('main-article__caption');

    image.src = item.image;
    image.alt = 'Фото новости';
    category.textContent = data.categories.find(({ id }) => item.category_id === id).name;
    text.textContent = item.description;
    title.textContent = item.title;
    source.textContent = data.sources.find(({ id }) => item.source_id === id).name;
...
const createSmallNewsItem = (item) => {
    ...
    title.textContent = item.title;
    source.textContent = data.sources.find(({ id }) => item.source_id === id).name;
    date.textContent = new Date(item.date).toLocaleDateString('ru-RU', { month: 'long', day: 'numeric' });
